...
class AdminController {
    ...
    public static function dashboard(): void {
        ...
        // Stats luôn tính từ toàn bộ data (không bị ảnh hưởng bởi filter status/search)
        $allLicenses = ($search === '' && $status === '' && $agency === '') ? $licenses : LicenseModel::all();
        $now     = time();
        $total   = count($allLicenses);
        $active  = count(array_filter($allLicenses, fn($l) => !$l['revoked'] && strtotime($l['expiry_date']) > $now));
        $expired = count(array_filter($allLicenses, fn($l) => !$l['revoked'] && strtotime($l['expiry_date']) <= $now));
        $revoked = count(array_filter($allLicenses, fn($l) => $l['revoked']));
