...
<head>
  ...
  <style>
    ...
    .no-email-row { margin:10px 0 6px; display:flex; align-items:center; gap:8px; }
    .no-email-row input[type="checkbox"] { width:auto; margin:0; accent-color:#3b82f6; }
    .no-email-row .form-label { margin:0; font-weight:500; }
  </style>
</head>
<body>
<div class="box">
  ...
  <?php elseif ($msg === 'email_required'): ?>
    <div class="alert alert-error">Key này đã gắn email. Vui lòng bỏ chọn "Không có email" và nhập email đúng để reset.</div>
  ...
  <form method="POST" action="/reset-device">
    ...
    <div class="no-email-row">
      <input id="no_email" type="checkbox" name="no_email" value="1" <?php echo !empty($form['no_email']) ? 'checked' : ''; ?>>
      <label for="no_email" class="form-label">Không có email</label>
    </div>
...
<script>
  (function () {
    const cb = document.getElementById('no_email');
